...
class ConsumerFileTest extends PHPUnit_Framework_TestCase
{
    ...
    function setUp()
    {
        ...
        $this->client = new Client("oq0vdlg7yi", array(
                "consumer" => "file",
    ...
    function testSend()
    {
        ...
        exec("php --define date.timezone=UTC send.php --secret oq0vdlg7yi --file /tmp/analytics.log",
            $output);

        $this->assertEquals("sent 200 from 200 requests successfully",
            trim($output[0]));
    ...
    function testProductionProblems()
    {
        ...
        $client = new Client("oq0vdlg7yi", array(
                "consumer" => "file",
                "filename" => "/dev/xxxxxxx"
            ));

        $tracked = $client->track(array(
            "userId" => "some-user",
            "event" => "my event"
        ));
